...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* Elabora RX Ethernet + timer LwIP (nessun RTOS: va chiamata a polling
     * dal loop principale, altrimenti lo stack di rete non processa mai
     * nulla e nessuna connessione TCP puo' funzionare). */
    MX_LWIP_Process();

    /* Poll RTD, PID, safety check, watchdog di disconnessione, PWM */
    Heater_Service();

    /* Heartbeat: LED verde lampeggia ogni 500 ms se il loop gira */
 ...
 *         Voltage Scale 0 richiesto per 480 MHz su STM32H753.
